<script lang="ts">
	...
	async function resetBrowserData() {
		const confirmed = window.confirm(
			'Reset SahelWatch on this browser? You will be logged out and first-time setup will appear again. This does not delete your phone account or change your public IP address.'
		);
		if (!confirmed) return;
		settingsBusy = true;
		try {
...
<div class="app-shell">
	...
	<main id="main-content">
		{#if !fetchingPrediction && activeTab === 'overview'}
			...
			<label class="mobile-day-picker glass">
				<span>Forecast day</span>
				<select bind:value={selectedDay} aria-label="Choose an outlook day">
					<option value="today">Today · {today}</option>
					<option value="tomorrow">Tomorrow · {nextOutlookDate}</option>
					<option value="day-after">Day after tomorrow · {dayAfterDate}</option>
				</select>
			</label>
		{/if}
		{#if fetchingPrediction && activeTab === 'overview'}
		...
		{:else if activeTab === 'overview'}
			<section class="mobile-forecast glass" aria-label="Selected dust outlook">
				<div class="mobile-forecast-top">
					<div><p class="eyebrow">{selectedDay === 'today' ? 'Today' : selectedDay === 'tomorrow' ? 'Tomorrow' : 'Day after tomorrow'} · {selectedTargetDate}</p><h1>{riskCopy}</h1><p>{selected.name}, {selected.country}</p></div>
					<div class="compact-risk {riskTone}" style={`--value:${activePrediction.probability}`}>
						<strong>{probability}</strong><span>%</span><small>{activePrediction.riskLevel}</small>
					</div>
				</div>
				<div class="mobile-metrics">
					<div><span><Wind size={17}/></span><small>Wind</small><strong>{conditions?.windSpeedKmh != null ? `${conditions.windSpeedKmh} km/h` : 'Unavailable'}</strong></div>
					<div><span><Thermometer size={17}/></span><small>Temperature</small><strong>{conditions?.temperatureC != null ? `${conditions.temperatureC.toFixed(2)}°C` : 'Unavailable'}</strong></div>
					<div><span><Droplets size={17}/></span><small>Soil moisture</small><strong>{soilMoistureValue != null ? `${(soilMoistureValue * 100).toFixed(1)}%` : 'Unavailable'}</strong></div>
					<div><span><Gauge size={17}/></span><small>Particles (AOD)</small><strong>{aodAvailable && aodValue != null ? aodValue.toFixed(2) : 'Unavailable'}</strong></div>
				</div>
			</section>
			...
			<section class="metrics" aria-label="Environmental forecast indicators">
				...
				<article class="glass"><span class="metric-icon"><Thermometer size={20}/></span><div><p>Temperature</p><strong>{conditions?.temperatureC != null ? `${conditions.temperatureC.toFixed(2)}°C` : 'Unavailable'}</strong><small>{temperatureExplanation(conditions?.temperatureC)}</small></div></article>
		...
		{:else if activeTab === 'tracking'}
			...
			<label class="mobile-day-picker tracking-picker glass">
				<span>Tracking day</span>
				<select bind:value={selectedDay} aria-label="Choose a tracking day">
					<option value="today">Today · {today}</option>
					<option value="tomorrow">Tomorrow · {nextOutlookDate}</option>
					<option value="day-after">Day after tomorrow · {dayAfterDate}</option>
				</select>
			...
			{:else}
				<section class="tracking-grid">
					<div class="tracking-map glass">{#if PredictionMapComponent}<svelte:component this={PredictionMapComponent} location={selected} prediction={activePrediction} {conditions} dark={darkTheme}/>{/if}</div>
	...
	<footer class="glass"><span>SahelWatch</span><p>Low-cost environmental intelligence for the African Sahel.</p><small>Research forecast · Not an official emergency warning</small></footer>
</div>
...
<style>
	...
	.tabs button.active { color: var(--on-brand); border:1px solid rgba(255,255,255,.36); background:linear-gradient(145deg,#759fc6,var(--brand-strong)); box-shadow:0 8px 24px color-mix(in srgb,var(--blue) 25%,transparent),inset 0 1px rgba(255,255,255,.55); }
	...
	.mobile-day-picker,.mobile-forecast { display:none; }
	...
	:global(:root:not([data-theme='dark'])) .hero-copy { width:min(820px,100%); padding:clamp(24px,3vw,40px); border:1px solid rgba(255,255,255,.64); border-radius:var(--radius-xl); background:linear-gradient(145deg,rgba(221,232,250,.76),rgba(181,199,232,.58)); box-shadow:0 24px 70px rgba(18,31,72,.22),inset 0 1px rgba(255,255,255,.82); backdrop-filter:blur(30px) saturate(145%); -webkit-backdrop-filter:blur(30px) saturate(145%); }
	...
	.primary { color: var(--on-brand); background:linear-gradient(135deg,#739dc3,var(--brand-strong)); box-shadow:0 12px 30px color-mix(in srgb,var(--blue) 31%,transparent),inset 0 1px rgba(255,255,255,.58); }.primary:hover { transform: translateY(-1px); box-shadow:0 16px 38px color-mix(in srgb,var(--blue) 38%,transparent),inset 0 1px rgba(255,255,255,.62); }.primary:disabled { opacity: .55; cursor: wait; }
	.secondary { border: 1px solid var(--glass-border); background:color-mix(in srgb,var(--surface-solid) 32%,transparent); box-shadow:inset 0 1px rgba(255,255,255,.3); backdrop-filter:blur(18px) saturate(170%); }
	...
	@media (max-width: 620px) {
		...
		.mobile-day-picker { position:relative; margin:12px 0 10px; padding:9px 12px; display:grid; grid-template-columns:auto 1fr; align-items:center; gap:12px; border-radius:20px; }
		.mobile-day-picker > span { color:var(--text-secondary); font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.06em; }
		.mobile-day-picker select { width:100%; min-height:44px; padding:0 36px 0 13px; border:1px solid var(--glass-border); border-radius:14px; color:var(--text); background:color-mix(in srgb,var(--surface-solid) 94%,transparent); box-shadow:inset 0 1px rgba(255,255,255,.48); backdrop-filter:blur(24px); font-size:.84rem; font-weight:700; }
		.mobile-forecast { margin:0 0 14px; padding:16px; display:block; border-radius:28px; }
		.mobile-forecast-top { display:grid; grid-template-columns:minmax(0,1fr) 104px; align-items:center; gap:14px; }
		.mobile-forecast-top h1 { margin:0 0 6px; font-size:clamp(1.65rem,8vw,2.25rem); line-height:1.02; letter-spacing:-.05em; }
		.mobile-forecast-top > div:first-child > p:last-child { margin:0; color:var(--text-secondary); font-size:.78rem; }
		.compact-risk { --tone:var(--green); width:104px; aspect-ratio:1; display:grid; place-content:center; justify-items:center; border-radius:50%; background:radial-gradient(circle at center,var(--bg-elevated) 0 58%,transparent 59%),conic-gradient(var(--tone) calc(var(--value) * 1turn),color-mix(in srgb,var(--tone) 13%,transparent) 0); box-shadow:0 16px 36px color-mix(in srgb,var(--tone) 14%,transparent); }
		.compact-risk.watch { --tone:var(--yellow); }.compact-risk.warning { --tone:var(--orange); }.compact-risk.alert { --tone:var(--red); }
		.compact-risk strong { font-size:2.15rem; line-height:.8; font-variant-numeric:tabular-nums; }.compact-risk > span { color:var(--text-secondary); font-size:.72rem; }.compact-risk small { margin-top:4px; color:var(--tone); font-size:.58rem; font-weight:800; text-transform:uppercase; }
		.mobile-metrics { margin-top:15px; display:grid; grid-template-columns:1fr 1fr; gap:8px; }
		.mobile-metrics > div { min-width:0; padding:11px; display:grid; grid-template-columns:26px 1fr; align-items:center; border:1px solid color-mix(in srgb,var(--glass-border) 86%,var(--border)); border-radius:16px; background:color-mix(in srgb,var(--surface-solid) 88%,transparent); box-shadow:inset 0 1px rgba(255,255,255,.34),0 8px 22px rgba(15,28,42,.08); }
